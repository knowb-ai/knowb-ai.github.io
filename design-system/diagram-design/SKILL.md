---
name: diagram-design
description: Create KnowB technical and product diagrams — architecture, IT current-state, flowchart, sequence, state machine, ER / data model, timeline, swimlane, quadrant, radar / spider, loop / flywheel, nested, tree, org chart, layer stack, venn, pyramid / funnel, bar chart, line chart, Gantt, scatter plot, high-level, process, medallion, data flow, DP integration, DP security matrix — as standalone HTML files with inline SVG. Uses the KnowB Autumn editorial skin and includes annotation-callout and optional sketchy primitives.
license: MIT
metadata:
  version: "2.0"
---
...
This is KnowB AI Systems' maintained adaptation of Cathryn Lavery's MIT-licensed Diagram Design system. Provenance, license, and the upstream revision are recorded in [`UPSTREAM.md`](UPSTREAM.md).

The brand gate has already been completed for this repository. Use [`references/style-guide.md`](references/style-guide.md) as the binding visual source of truth and use the local templates in `assets/` or `static/diagrams/` as the starting point. Do not reintroduce upstream default colors, alternate hues, or unapproved gradients.

---

## 0. First-time setup — style guide gate

**The KnowB Autumn style guide is already customized and must be used as-is.**

Read [`references/style-guide.md`](references/style-guide.md) and [`ORGBRAND_GUIDELINES.md`](../../ORGBRAND_GUIDELINES.md) before producing a diagram. If the requested output needs a new semantic role or a color outside the approved eight-color palette, pause for an explicit brand decision instead of inventing it.
...
- The focal accent is **editorial, not a flag.** 1–2 focal nodes per diagram. Using it on 5 nodes erases the signal.
...
| JetBrains Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names go in Inter. |
...
| Accent on every "important" node | Accent is for 1–2 editorial focal points, not a signaling system |
...
- **Title** — Space Grotesk, 1.75rem, 600 — H1 only
- **Node name** — Inter (sans), 12px, 600 — human-readable labels
- **Sublabel** — JetBrains Mono, 9px — ports, URLs, field types
- **Eyebrow / tag** — JetBrains Mono, 7–8px, uppercase, tracked — type tags, axis labels
- **Arrow label** — JetBrains Mono, 8px — annotation on arrows
- **Editorial aside** — Space Grotesk *italic*, 14px — callouts only

**Mono is for technical content.** Names are Inter. Page titles and editorial callouts use Space Grotesk. Never JetBrains Mono as a blanket "dev" font.
...
```svg
<rect width="100%" height="100%" fill="#FCF6F3"/>
```
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="1" cy="1" r="0.9" fill="rgba(122,34,23,0.10)"/>
  </pattern>
</defs>
<rect width="100%" height="100%" fill="#FCF6F3"/>
...
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#77584F"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#8E2F2A"/>
</marker>
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#7A2217"/>
</marker>
```

| Arrow | Stroke | When |
|---|---|---|
| Default | muted `#77584F` | Internal, generic |
| Accent | Deep Rust `#8E2F2A` | Primary / highlighted / headline |
| Link | Charcoal Bark `#7A2217` light, Saber Blue `#86E8FF` dark | HTTP/API calls, external systems |
...
<rect x="X" y="Y" width="W" height="H" rx="6" fill="#FCF6F3"/>
<!-- 2. Styled box -->
...
<text x="X+22" y="Y+15" fill="STROKE@0.8" font-size="7" font-family="'JetBrains Mono', monospace"
      text-anchor="middle" letter-spacing="0.08em">API</text>
<!-- 4. Node name (Inter — human-readable) -->
<text x="CX" y="CY+2" fill="#7A2217" font-size="12" font-weight="600"
      font-family="Inter, sans-serif" text-anchor="middle">Node Name</text>
<!-- 5. Technical sublabel (JetBrains Mono) -->
<text x="CX" y="CY+18" fill="#77584F" font-size="9"
      font-family="'JetBrains Mono', monospace" text-anchor="middle">tech:port</text>
```
...
<rect x="MID_X-18" y="ARROW_Y-20" width="36" height="12" rx="2" fill="#FCF6F3"/>
<text x="MID_X" y="ARROW_Y-11" fill="#77584F" font-size="8"
      font-family="'JetBrains Mono', monospace" text-anchor="middle" letter-spacing="0.06em">WRITE</text>
```
...
<text x="30" y="LEGEND_Y+8" fill="#77584F" font-size="8" font-family="'JetBrains Mono', monospace"
...
1. **Header** — eyebrow (JetBrains Mono), title (Space Grotesk), optional subtitle (Inter muted).
...
4. **Footer** — colophon in JetBrains Mono, muted, hairline top border.
...
- [ ] Every arrow label has an opaque `paper`-color rect behind it?
...
- [ ] Human-readable names in Inter, not JetBrains Mono?
- [ ] Technical sublabels (ports, commands, URLs) in JetBrains Mono?
- [ ] Page title in Space Grotesk?
- [ ] Annotation callouts (if any) in *italic* Space Grotesk? (see [primitive-annotation.md](references/primitive-annotation.md))
